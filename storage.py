...
def update_perfume(perfume_id: str, changes: Dict[str, Any]) -> bool:
    ...
    for i, it in enumerate(items):
        if it.get("id") == perfume_id:
            ...
            if "price" in it: it["price"] = float(it["price"])
            if "rating" in it: it["rating"] = float(it["rating"])
            if "stock" in it: it["stock"] = int(it["stock"])
            if "notes" in it and not isinstance(it["notes"], list):
                it["notes"] = [str(x).strip() for x in str(it["notes"]).split(",") if str(x).strip()]
            if "allergens" in it and not isinstance(it["allergens"], list):
                it["allergens"] = [str(x).strip() for x in str(it["allergens"]).split(",") if str(x).strip()]
            items[i] = it
            found = True
            break
    if found:
...
def seed_minimal() -> int:
    ...
    samples = [
        {"id": "seed-rose-dusk", "name": "Rose Dusk", "brand": "Floral", "price": 55.0, "notes": ["rose","musk"], "allergens": [], "rating": 4.2, "stock": 5},
        {"id": "seed-citrus-day", "name": "Citrus Day", "brand": "FreshCo", "price": 42.0, "notes": ["lemon","neroli"], "allergens": [], "rating": 4.0, "stock": 8},
        {"id": "seed-night-bloom-xl", "name": "Night Bloom XL", "brand": "Nocturne", "price": 65.0, "notes": ["jasmine","amber","musk"], "allergens": ["coumarin"], "rating": 4.4, "stock": 7},
    ]
    _save(samples)
    return len(samples)
    ...
    for i, it in enumerate(items):
        if it.get("id") == pid:
            ...
            if "notes" in items[i] and not isinstance(items[i]["notes"], list):
                items[i]["notes"] = [s.strip() for s in str(items[i]["notes"]).split(",") if s.strip()]
            if "allergens" in items[i] and not isinstance(items[i]["allergens"], list):
                items[i]["allergens"] = [s.strip() for s in str(items[i]["allergens"]).split(",") if s.strip()]
            _save(items)
            return True, None
    # not found -> add
    add_perfume(item)
